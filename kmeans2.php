...
while($status=='false') {
	...
	while ($data=mysqli_fetch_assoc($result)) {
		...
		//Operasi
		for ($i=1;$i<=$jumlahcluster;$i++) { 
			$hasilc[$i]=sqrt(pow($credit-$cluster[$i][0],2)+pow($duration-$cluster[$i][1],2));
		}
	...
	$loop+=1;
	for ($i=1;$i<=$jumlahcluster; $i++) {
		$cluster[$i][0]=caridata($mysqli,"select avg(credit) from tb_transformasi where c_2variabel='C$i'");
		$cluster[$i][1]=caridata($mysqli,"select avg(duration) from tb_transformasi where c_2variabel='C$i'");
	}
...
echo "Loop : ".$loop;
echo "<br>";
$x=caridata($mysqli,"SELECT SUM((value_x-credit)*(value_x-credit)) FROM tb_transformasi JOIN tb_centro_new ON centro=c_2variabel where number='$jumlahcluster'");
$y=caridata($mysqli,"SELECT SUM((value_y-duration)*(value_y-duration)) FROM tb_transformasi JOIN tb_centro_new ON centro=c_2variabel where number='$jumlahcluster'");
echo "<br>";
echo ($x+$y);
echo "<br>";
//}
